...
public class CommandFarmHelp extends SFCommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, String label, String[] args) {
        ...
        for (int i = (page - 1) * PER_PAGE; i < Math.min(helps.size(), page * PER_PAGE); i++) {
            ...
            sender.sendMessage(Reference.PREFIX + " - " + help.description);
        }
        return true;
    }
    ...
    private enum Help {
        ...
        FARM_OP_SET_MAX_USER("/팜 파티 인원설정 <닉네임> <최대 인원>", "특정 유저의 최대 인원을 설정합니다.", true);
        ...
        Help(String usage, String description) {
            this(usage, description, false);
        }

        Help(String usage, String description, boolean requiredOp) {
            this.usage = usage;
